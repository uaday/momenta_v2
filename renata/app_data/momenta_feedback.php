...
if(mysqli_query($conn,$sql1))
{
	...
	while($row=mysqli_fetch_assoc($result))
	{
		...
		{
			$curl = curl_init();
			curl_setopt_array($curl, array( CURLOPT_RETURNTRANSFER => 1, CURLOPT_URL => 'http://sms.sslwireless.com/pushapi/dynamic/server.php?user=RenataPharmaceuticals&pass=92o<8H52&sid=Momenta&sms='.urlencode("Momenta Applicaton Feedback Form: tinyurl.com/momenta-feedback").'&msisdn=88'.$pso_phone.'&csmsid='.$number.'App'.$pso_renata_id.'',CURLOPT_USERAGENT => 'Sample cURL Request' ));
            $resp = curl_exec($curl);
            if($resp)
            {
            	echo "ok";
