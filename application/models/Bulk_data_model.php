        ...
        if (!in_array($ext, $allowed)) {
            ...
            while ($csv_line = fgetcsv($fp, 1024)) {
                if (count($csv_line) == 10) {
                    $insert_csv = array();
                    $insert_csv['sm_code'] =$this->db->escape_str($csv_line['0']);
                    $insert_csv['rsm_code'] =$this->db->escape_str($csv_line['1']);
                    $insert_csv['dsm_code'] =$this->db->escape_str($csv_line['2']);
                    $insert_csv['renata_id'] =$this->db->escape_str($csv_line['3']);
                    $insert_csv['name'] =$this->db->escape_str($csv_line['4']);
                    $insert_csv['designation'] =$this->db->escape_str($csv_line['5']);
                    $insert_csv['depot_code'] =$this->db->escape_str($csv_line['6']);
                    $insert_csv['b_code'] =$this->db->escape_str($csv_line['7']);
